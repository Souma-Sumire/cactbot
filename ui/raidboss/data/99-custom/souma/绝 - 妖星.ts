...
const getP2 = (data: Data, _matches: Matches, output: Output) => {
  ...
  if (data.p2count === 1) {
    if (Util?.souma?.getRpByName !== undefined) {
      ...
      const partnerMap: Partial<Record<RP, RP>> = data.triggerSetConfig.p2分组 === 'MTH1'
        ? {
          'MT': 'H1',
          'H1': 'MT',
          'ST': 'H2',
...
const markerCodes: Record<string, string> = {
  '0': '攻击1',
  '1': '攻击2',
  '2': '攻击3',
  '3': '攻击4',
  ...
  triggers: [
    ...
    },
    {
      ...
      infoText: (data, matches, output) => {
        ...
        const dir1 = Directions.hdgTo8DirNum(c1);
        const p = (clk !== '顺' ? +1 : -1);
        const macro: { mj: number; wy: string[] }[] = [];
        for (let i = 1; i <= 8; i++) {
      ...
      return {
        ...
        infoText: (data, _matches, output) => {
          ...
          return output.text!().replaceAll(
            ...
            const num = { '攻击': 3, '锁链': 3, '禁止': 2 }[type as '攻击' | '锁链' | '禁止'];
            const res = [];
            for (let i = 1; i <= num; i++) {
